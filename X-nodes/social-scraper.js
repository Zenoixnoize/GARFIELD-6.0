...
if (cn.WORKTYPE == 'private') {
    ...
    Garfield.addXnodes({ pattern: 'insta ?(.*)', fromMe: true, desc: Lang.DESC }, (async (message, match) => {
    ...
    /*
    Garfield.addXnodes({ pattern: 'tiktok ?(.*)', fromMe: true, desc: Tlang.TİKTOK }, async (message, match) => {
        ...
        await axios
          ...
          .then(async (response) => {
            ...
            await message.sendMessage(Buffer.from(profileBuffer.data), MessageType.video, {
              caption: ' ```Follow Us Facebook - https://www.facebook.com/garfieldbots/``` ',
            })
          })
          .catch(
    ...
    */
    ...
    Garfield.addXnodes({ pattern: 'insta ?(.*)', fromMe: false, desc: Lang.DESC }, (async (message, match) => {
        if (match[0].includes('install')) return;
        if (match[1] === '') return await message.client.sendMessage(message.jid, Lang.NEED_WORD, MessageType.text, { quoted: message.data });
        if (!match[1].includes('www.instagram.com')) return await message.client.sendMessage(message.jid, Lang.NEED_WORD, MessageType.text, { quoted: message.data });
    ...
    Garfield.addXnodes({ pattern: 'insta ?(.*)', fromMe: true, desc: Lang.DESC, dontAddCommandList: true }, (async (message, match) => {
        if (match[0].includes('install')) return;
        if (match[1] === '') return await message.client.sendMessage(message.jid, Lang.NEED_WORD, MessageType.text, { quoted: message.data });
        if (!match[1].includes('www.instagram.com')) return await message.client.sendMessage(message.jid, Lang.NEED_WORD, MessageType.text, { quoted: message.data });
    ...
    /*
    Garfield.addXnodes({ pattern: 'tiktok ?(.*)', fromMe: false, desc: Tlang.TİKTOK }, async (message, match) => {
