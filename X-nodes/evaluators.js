...
Garfield.addXnodes({pattern: 'print ?(.*)', fromMe: true, desc: dd}, (async (message, match) => {    
...
Garfield.addXnodes({pattern: 'bashmedia ?(.*)', fromMe: wk_q, desc: bdesc, usage: 'video.mp4 && media/gif/pic.mp4'}, (async (message, match) => {    
    var id = message.jid
    try {
        if (match[1].includes('jpg') || match[1].includes('tiff') || match[1].includes('raw') || match[1].includes('dng') || match[1].includes('png') || match[1].includes('jpeg')) {
            await message.client.sendMessage(id,fs.readFileSync(`/root/GARFIELD-6.0/${match[1]}`), MessageType.image, {caption: ' ```Follow Us Facebook - https://www.facebook.com/garfieldbots/``` ' })
        }
        else if (match[1].includes('mp4') || match[1].includes('avi') || match[1].includes('webm') || match[1].includes('mkv') || match[1].includes('mpeg')) {
            await message.client.sendMessage(id,fs.readFileSync(`/root/GARFIELD-6.0/${match[1]}`), MessageType.video, {caption: ' ```Follow Us Facebook - https://www.facebook.com/garfieldbots/``` ' });
        }
            ...
            await message.client.sendMessage(id,fs.readFileSync(`/root/GARFIELD-6.0/${match[1]}`), MessageType.audio);
        }
        else {
...
Garfield.addXnodes({pattern: 'addserver$', fromMe: wk_ad, desc: addsdesc}, (async (message, match) => {    
    if (message.reply_message.image) {
        ...
        exec('mv ' + location + ' /root/GARFIELD-6.0/server-image.' + fin)
        ...
        var fin = location.split('.')[1]
        exec('mv ' + location + ' /root/GARFIELD-6.0/server-video.' + fin)
        await message.client.sendMessage(message.jid,suc_add, MessageType.text)
    }
        ...
        var fin = location.split('.')[1]
        exec('mv ' + location + ' /root/GARFIELD-6.0/server-audio.' + fin)
        await message.client.sendMessage(message.jid,suc_add, MessageType.text)
    }
...
Garfield.addXnodes({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {
...
Garfield.addXnodes({pattern: 'term ?(.*)', fromMe: true, desc: Lang.TERM_DESC}, (async (message, match) => {    
...
Garfield.addXnodes({pattern: 'mediainfo$', fromMe: wk, desc: medinfo}, (async (message, match) => {    
    var id = message.jid
    if (message.reply_message.video) {
        ...
        exec('mv ' + location + ' /root/GARFIELD-6.0/vid.mp4')
        exec('ffprobe -hide_banner -loglevel fatal -show_error -show_format -show_streams -show_programs -show_chapters -show_private_data -print_format json /root/GARFIELD-6.0/vid.mp4', async (err, st, stderr) => {
            if (err) {
...
Garfield.addXnodes({pattern: 'pmsend ?(.*)', fromMe: true, desc: pmmm }, (async (message, match) => {
...
Garfield.addXnodes({pattern: 'pmttssend ?(.*)', fromMe: true, desc: psmm}, (async (message, match) => {
    if (!message.reply_message) return await message.client.sendMessage(message.jid,NLang.NEED_REPLY, MessageType.text);
    if (message.reply_message && match[1] == '') return await message.client.sendMessage(message.jid, NLang.NEED_WORDS, MessageType.text);
    let 
